...
func TestRowToJsonSuccess(t *testing.T) {
	...
	var jsonRowMap map[string]interface{}
	err = json.Unmarshal(expectedResult.JsonRow, &jsonRowMap)
	if err != nil {
		t.Errorf("Error unmarshalling expectedResult.JsonRow %s", err)
	}

	//validate transformed JSON row
	if !reflect.DeepEqual(jsonRowMap, actualTransformedResult["json_row"]) {
		t.Errorf("Expected jsonRow  %v and actual  %v do not match", jsonRowMap, actualTransformedResult["json_row"])
	}
